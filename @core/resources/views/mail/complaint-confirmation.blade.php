...
<head>
    ...
    <style>
        * { font-family: 'Open Sans', sans-serif; }
        ...
        .inner-wrap p { font-size: 16px; line-height: 26px; color: #656565; margin: 0; margin-bottom: 20px; }
        table { margin: 0 auto; border-collapse: collapse; width: 100%; }
        table td, table th { border: 1px solid #ddd; padding: 8px; text-align: left; }
        table tr:nth-child(even) { background-color: #f2f2f2; }
        table th { padding-top: 12px; padding-bottom: 12px; background-color: #008649; color: white; }
...
<div class="mail-container">
    ...
    <div class="inner-wrap">
        <p>{{ __('Dear') }} <strong>{{ $complaint->full_name }}</strong>,</p>
        <p>{{ __('Thank you for submitting your complaint to Uttara Bank PLC. We have received it and will review it shortly.') }}</p>
        <p>{{ __('Please find below a summary of your submission for your reference:') }}</p>
        <table>
            ...
            <tr><td>{{ __('Suggestion') }}</td><td>{{ $complaint->suggestion }}</td></tr>
        </table>
        <p style="margin-top: 20px;">{{ __('Our team will contact you soon. Thank you for trusting Uttara Bank PLC.') }}</p>
    </div>
    <footer>
