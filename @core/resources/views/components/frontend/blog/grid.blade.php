<div class="blog-classic-item-01 {{$margin ? 'margin-bottom-60' : ''}}">
    ...
    <div class="content">
        ...
        <h4 class="title"><a href="{{route('frontend.news.single',$blog->slug)}}">{{$blog->title}}</a></h4>
        <p>{{$blog->excerpt}}</p>
        <div class="btn-wrapper">
            <a href="{{route('frontend.news.single',$blog->slug)}}" class="boxed-btn reverse-color">{{get_static_option('blog_page_'.$user_select_lang_slug.'_read_more_btn_text')}}</a>
        </div>
    </div>
</div>