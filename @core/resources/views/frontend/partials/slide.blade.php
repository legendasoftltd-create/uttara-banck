        ...
        <div id class="banner-carousel owl-carousel owl-theme">
            @foreach ($all_header_slider as $data)
                <div class="slide-item d-md-flex justify-content-between align-items-center">
                    ...
                    @endphp
                    <img src="{{ $image_details['img_url'] }}" alt="">
                    {{-- <img src="assets/images/banner/hero_banner_6.webp" alt=""> --}}
                    <div class="background-overlay"></div>
                    <div class="slider-text">
                        @if (!empty($data->title))
                            <h1 class="title-primary m-0">{{ $data->title }}</h1>
                        @endif
                        @if (!empty($data->subtitle))
                            <p class="mb-md-3 m-0">{{ $data->subtitle }}</p>
                        @endif
