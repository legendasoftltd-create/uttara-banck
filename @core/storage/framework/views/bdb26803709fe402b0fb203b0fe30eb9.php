...
<?php $__env->startSection('script'); ?>
    <script>
        $(document).ready(function () {
            ...
            function getSlugCheckType($input) {
                return $input.closest('#brand_edit_modal_form').length ? 'update' : 'new';
            }

            function syncSlug($input, shouldAutoFill) {
                var form = $input.closest('form');
                var titleField = form.find('input[name="title"]');
                var slugField = form.find('input[name="slug"]');
                var type = getSlugCheckType($input);

                if (shouldAutoFill) {
                    if (type === 'update' && slugField.val() !== '') {
                        return;
                    }
                    slugField.val(convertToSlug(titleField.val()));
                } else {
                    slugField.val(convertToSlug(slugField.val()));
                }
