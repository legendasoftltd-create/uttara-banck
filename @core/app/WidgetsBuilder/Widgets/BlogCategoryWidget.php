...
class BlogCategoryWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        foreach ($blog_posts as $post) {
            $output .= '<li><a href="' . route('frontend.news.category', ['id' => $post->id,'any' => Str::slug(purify_html($post->name))]) . '"> <i class="fas fa-chevron-'.$icon_class.'"></i>' . purify_html($post->name) . '</a></li>';
        }
        $output .= '</ul>';
        $output .= '</div>';
