...
class MediaUploadController extends Controller
{
    ...
    {
        $image_extension = strtolower($image->extension());
        $isGif = $image_extension === 'gif';

        if ($isGif) {
            // Keep the animated GIF as-is for the main uploaded file
            $image->move($folder_path, $image_db);
            $image_path = $folder_path . $image_db;
        } else {
        ...
        $image_height = $image_dimension[1];
        $image_dimension_for_db = $image_width . ' x ' . $image_height . ' pixels';
        $image_size_for_db = $isGif ? filesize($image_path) : $image->getSize();

        $image_grid = 'grid-' . $image_db;
        $image_large = 'large-' . $image_db;
        $image_thumb = 'thumb-' . $image_db;

        $resize_grid_image = Image::make($image_path)->resize(350, null, function ($constraint) {
            $constraint->aspectRatio();
        });
        $resize_large_image = Image::make($image_path)->resize(740, null, function ($constraint) {
            $constraint->aspectRatio();
        });
        $resize_thumb_image = Image::make($image_path)->resize(150, 150);

        if (! $isGif) {
            $resize_full_image = Image::make($image_path)->resize($image_width, $image_height, function ($constraint) {
                $constraint->aspectRatio();
            });
            $resize_full_image->save($folder_path . $image_db);
        }
