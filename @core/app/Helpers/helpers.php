...
function get_mege_menu_item_url($type, $slug)
{
    ...
    switch ($type) {
        ...
        case('donation_mega_menu'):
            ...
            break;
        case('blog_mega_menu'):
            $return_val =  route('frontend.news.single',$slug);
            break;
        case('job_mega_menu'):
...
function get_blog_category_by_id($id,$type = '',$class = ''){
    ...
    if (!empty($blog_cat)){
        ...
        if ($type == 'link' ){
            $return_val = '<a class="'.$class.'" href="'.route('frontend.news.category',['id' => $blog_cat->id,'any' => Str::slug($blog_cat->name,'-',null) ]).'">'.$blog_cat->name.'</a>';
        }
    }
...
function get_category_products($id){
    $products = Products::where(['status' => 'publish', 'category_id' => $id])->orderBy('id', 'desc')->paginate(get_static_option('product_post_items'));
    return $products;
}
...
function timeAgo($date) {
    ...
    if ($created->diffInMinutes($now) < 60) {
        return $created->diffInMinutes($now) . ' min';
    }

    if ($created->diffInHours($now) < 24) {
        return $created->diffInHours($now) . ' hour';
    }

    if ($created->diffInDays($now) <= 7) {
        return $created->diffInDays($now) . ' day';
    }
